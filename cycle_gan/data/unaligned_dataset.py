...
#        print('current_epoch', self.current_epoch)
        ...
        good_B_found_flag = False

        index_B = index
        try_counter = 0 # counter of tries to find a good patch on the initially picked image

        while not good_B_found_flag:
            B_path = self.B_paths[index_B % self.B_size]  # make sure index is within then range
            ...
            # check if A varience is high enough (if any cells are present)
            if np.var(np.array(B)) >= 0:
                good_B_found_flag = True

            try_counter+=1

            if try_counter > 3:
                index_B = np.random.randint(self.B_size)
#        print('1')
#        print(np.array(A_img).shape, np.array(B_img).shape)
        B = convert_transform(B)
